...
test("builds and smoke-tests the Codespaces development environment", async () => {
  ...
  assert.equal(devcontainer.features["ghcr.io/devcontainers/features/rust:1"].version, "stable");
...
test("publishes the persistent standalone application and Rust authentication container", async () => {
  ...
  assert.match(dockerfile, /cargo build --release --locked --manifest-path server\/Cargo\.toml/);
  assert.match(dockerfile, /scalengi-views-auth/);
  assert.match(dockerfile, /USER node/);
  assert.match(dockerfile, /HEALTHCHECK/);
  assert.match(dockerfile, /CMD \["bash", "\.\/start-container\.sh"\]/);
