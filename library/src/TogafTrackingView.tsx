...
export function TogafTrackingView({ data, configuration }: { data: ViewDataset; configuration?: ViewConfiguration }) {
  ...
  return <section ref={frameRef} className="view-workspace togaf-workspace togaf-adm-workspace">
    <header className="togaf-header"><div><p className="eyebrow">Architecture Development Method</p><h1>TOGAF ADM</h1></div><div className="togaf-header-metrics"><span><Route size={14} /><b>{averageProgress}%</b> {t("global")}</span><span className={gateReady ? "review" : ""}><ShieldCheck size={14} /><b>{gateReady}</b> gates</span><span className={blockers.length ? "danger" : ""}><AlertOctagon size={14} /><b>{blockers.length}</b> {t("alertes")}</span></div><div className="togaf-header-actions"><label className="filter-control"><select value={typeFilter} onChange={(event) => setTypeFilter(event.target.value)} aria-label={t("Filtrer les éléments TOGAF")}><option value="Tous">{t("Tous")}</option>{itemTypes.map((type) => <option key={type.id} value={type.id}>{type.label}</option>)}</select><ChevronDown size={14} /></label><button className="rf-fullscreen-button" onClick={() => void toggleFullscreen()}>{fullscreen ? <Minimize2 size={16} /> : <Maximize2 size={16} />}<span>{t(fullscreen ? "Quitter" : "Plein écran")}</span></button></div></header>
    <div className="adm-legend"><span><i className="done" /> {t("Validée")}</span><span><i className="active" /> {t("En cours")}</span><span><i className="review" /> {t("À valider")}</span><span><i className="blocked" /> {t("Bloquée")}</span><span><i className="todo" /> {t("À démarrer")}</span><em>{t("Cliquez sur une phase pour ouvrir son détail")}</em></div>
