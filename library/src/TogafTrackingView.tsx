...
const formatDate = (value: string, locale: "fr" | "en" = "fr") => value ? new Intl.DateTimeFormat(locale === "fr" ? "fr-FR" : "en-GB", { day: "2-digit", month: "short", year: "numeric" }).format(new Date(value)) : "—";
...
const AdmPhaseNode = memo(function AdmPhaseNode({ data, selected }: NodeProps<AdmPhaseData>) {
  ...
  return <article className={`adm-phase-node ${data.preliminary ? "preliminary" : ""} ${phaseStatusClass(phase.status)} ${selected ? "selected" : ""}`} title={`${phase.status} · ${data.items.length} ${locale === "fr" ? `élément${data.items.length > 1 ? "s" : ""}` : `item${data.items.length === 1 ? "" : "s"}`}`}>
...
const AdmHubNode = memo(function AdmHubNode({ data, selected }: NodeProps<AdmHubData>) {
  ...
  return <article className={`adm-hub-node ${selected ? "selected" : ""}`}>
    <Handle type="target" position={Position.Top} className="adm-handle" />
    <span className="adm-hub-kicker">{t("Au cœur de chaque phase")}</span>
    <Route size={22} />
    <strong>{data.label}</strong>
    <p>{data.phase?.objective || t("Les exigences sont identifiées, stockées et alimentent toutes les phases du cycle ADM.")}</p>
    <div><span><b>{data.progress}%</b> {t("global")}</span><span><b>{data.phaseCount}</b> {t("phases")}</span><span><b>{data.decisionCount}</b> {t("décisions")}</span><span className={data.riskCount ? "danger" : ""}><b>{data.riskCount}</b> {t("risques")}</span></div>
    <small>{t("Cliquez pour ouvrir le détail")}</small>
    <Handle type="source" position={Position.Bottom} className="adm-handle" />
  </article>;
});
...
export function TogafTrackingView({ data, configuration }: { data: ViewDataset; configuration?: ViewConfiguration }) {
  ...
  return <section ref={frameRef} className="view-workspace togaf-workspace togaf-adm-workspace">
    <ViewToolbar className="togaf-header" title="TOGAF ADM" icon={<Route size={16} />}><div className="togaf-header-metrics"><span><Route size={14} /><b>{averageProgress}%</b> {t("global")}</span><span className={gateReady ? "review" : ""}><ShieldCheck size={14} /><b>{gateReady}</b> gates</span><span className={blockers.length ? "danger" : ""}><AlertOctagon size={14} /><b>{blockers.length}</b> {t("alertes")}</span></div><div className="adm-legend"><span><i className="done" /> {t("Validée")}</span><span><i className="active" /> {t("En cours")}</span><span><i className="review" /> {t("À valider")}</span><span><i className="blocked" /> {t("Bloquée")}</span><span><i className="todo" /> {t("À démarrer")}</span></div><div className="togaf-header-actions"><label className="filter-control"><select value={typeFilter} onChange={(event) => setTypeFilter(event.target.value)} aria-label={t("Filtrer les éléments TOGAF")}><option value="Tous">{t("Tous")}</option>{itemTypes.map((type) => <option key={type.id} value={type.id}>{type.label}</option>)}</select><ChevronDown size={14} /></label><button className="rf-fullscreen-button toolbar-icon-button" aria-label={t(fullscreen ? "Quitter" : "Plein écran")} title={t(fullscreen ? "Quitter" : "Plein écran")} onClick={() => void toggleFullscreen()}>{fullscreen ? <Minimize2 size={16} /> : <Maximize2 size={16} />}</button></div></ViewToolbar>
    {!phases.length ? <div className="togaf-empty" data-view-export-content><Route size={32} /><h2>{t("Aucune trajectoire chargée")}</h2><p>{t("Définissez les phases dans la structure de la vue puis importez leurs éléments.")}</p></div> : <div className="adm-canvas" data-view-export-content><ReactFlowProvider><AdmCanvas phases={phases} items={items} typeLabel={typeLabel} selectedId={selectedId} onSelect={setSelectedId} fullscreen={fullscreen} progress={averageProgress} /></ReactFlowProvider></div>}
    {selected && <aside className="adm-detail-panel"><button className="panel-close" onClick={() => setSelectedId(null)} aria-label={t("Fermer le détail")}><X size={16} /></button><div className="adm-detail-heading"><span className={`phase-status ${phaseStatusClass(selected.status)}`}>{selected.status}</span><p>{selected.code}</p><h2>{selected.name}</h2><strong>{selected.progress}%</strong><i><b style={{ width: `${selected.progress}%` }} /></i></div><p className="adm-detail-objective">{selected.objective || t("Objectif à définir.")}</p><dl><div><UserRound size={14} /><dt>{t("Responsable")}</dt><dd>{selected.owner || t("Non défini")}</dd></div><div><CalendarClock size={14} /><dt>{t("Fenêtre")}</dt><dd>{formatDate(selected.startDate, locale)} → {formatDate(selected.targetDate, locale)}</dd></div><div><ShieldCheck size={14} /><dt>{t("Gate attendue")}</dt><dd>{selected.gate || t("Non définie")}</dd></div></dl><section><header><div><strong>{t("Éléments de la phase")}</strong><span>{typeFilter === "Tous" ? t("Tous les types") : typeLabel(typeFilter)}</span></div><em>{selectedItems.length}</em></header><div>{selectedItems.length ? selectedItems.map((item) => { const Icon = itemTypeIcon(item.type); return <article key={item.id} className={itemStatusClass(item.status)}><Icon size={14} /><div><span>{typeLabel(item.type)} · {item.status}</span><strong>{item.name}</strong><p>{item.detail || t("Aucun détail renseigné.")}</p><small>{item.owner || t("Sans responsable")} · {formatDate(item.dueDate, locale)}</small></div></article>; }) : <p className="adm-detail-empty">{t("Aucun élément avec ce filtre.")}</p>}</div></section></aside>}
    {requirementsSelected && <aside className="adm-detail-panel"><button className="panel-close" onClick={() => setSelectedId(null)} aria-label={t("Fermer le détail")}><X size={16} /></button><div className="adm-detail-heading requirements"><span className="phase-status active">{t("Transverse")}</span><p>ADM</p><h2>{requirements?.name ?? t("Gestion des exigences")}</h2><strong>{averageProgress}%</strong><i><b style={{ width: `${averageProgress}%` }} /></i></div><p className="adm-detail-objective">{requirements?.objective || t("Identifier, documenter, prioriser et maintenir les exigences qui alimentent toutes les phases du cycle ADM.")}</p><dl><div><Route size={14} /><dt>{t("Phases pilotées")}</dt><dd>{phases.filter((phase) => !isPreliminary(phase) && !isRequirements(phase)).length}</dd></div><div><Flag size={14} /><dt>{t("Décisions ouvertes")}</dt><dd>{allItems.filter((item) => item.type === "Décision" && item.status !== "Validé").length}</dd></div><div><AlertOctagon size={14} /><dt>{t("Risques ouverts")}</dt><dd>{allItems.filter((item) => item.type === "Risque" && item.status !== "Validé").length}</dd></div></dl><section><header><div><strong>{t("Éléments transverses")}</strong><span>{typeFilter === "Tous" ? t("Tous les types") : typeLabel(typeFilter)}</span></div><em>{requirementsItems.length}</em></header><div>{requirementsItems.length ? requirementsItems.map((item) => { const Icon = itemTypeIcon(item.type); return <article key={item.id} className={itemStatusClass(item.status)}><Icon size={14} /><div><span>{typeLabel(item.type)} · {item.status}</span><strong>{item.name}</strong><p>{item.detail || t("Aucun détail renseigné.")}</p><small>{item.owner || t("Sans responsable")} · {formatDate(item.dueDate, locale)}</small></div></article>; }) : <p className="adm-detail-empty">{t("Aucun élément directement rattaché. Les exigences restent néanmoins transverses à toutes les phases.")}</p>}</div></section></aside>}
  </section>;
}
