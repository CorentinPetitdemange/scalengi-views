...
export function PartitionView({ data, configuration }: ViewRendererProps) {
  ...
  if (!levels.length) return <div className="empty-state">Ajoutez au moins un niveau dans la structure de cette vue.</div>;
  ...
  return <section ref={frameRef} className="view-workspace rf-view-workspace partition-workspace">
    <header className="partition-header"><div><p className="eyebrow">Vue en découpage</p><h1>{configuration?.label ?? "Découpage personnalisé"}</h1></div><div className="partition-filters"><label className="search-control"><Search size={15} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Rechercher un élément…" /></label><button className={`partition-filter-button ${filtersOpen || activeFilterCount ? "active" : ""}`} onClick={() => setFiltersOpen((current) => !current)} aria-expanded={filtersOpen}><ListFilter size={15} /><span>Filtres</span>{activeFilterCount > 0 && <em>{activeFilterCount}</em>}</button></div><div className="partition-kpis">{displayedLevels.slice(0, 4).map((level) => <span key={level.id}><Layers3 size={14} /><small>{level.plural}</small><strong>{allItems.filter((item) => item.levelId === level.id).length}</strong></span>)}</div><button className="rf-fullscreen-button" onClick={() => void toggleFullscreen()}>{fullscreen ? <Minimize2 size={16} /> : <Maximize2 size={16} />}<span>{fullscreen ? "Quitter" : "Plein écran"}</span></button></header>
    {filtersOpen && <aside className="partition-filter-panel partition-condition-panel" data-export-exclude aria-label="Filtres de la vue"><header><div><ListFilter size={15} /><strong>Construire les filtres</strong></div><button onClick={() => setFiltersOpen(false)} aria-label="Fermer les filtres"><X size={15} /></button></header><section className="partition-filter-section"><span>Niveaux réellement visibles</span><div className="partition-filter-chips">{levels.filter((level) => level.role !== "reference").map((level) => <label key={level.id}><input type="checkbox" checked={levelFilters.includes(level.id)} onChange={() => setLevelFilters((current) => current.includes(level.id) ? current.filter((id) => id !== level.id) : [...current, level.id])} /><i style={{ background: level.color }} />{level.plural}</label>)}</div><small className="partition-filter-help">Sans sélection, tous les niveaux sont visibles. Un niveau masqué disparaît réellement, même s’il est parent d’un autre.</small></section><div className="partition-filter-mode"><span>Les éléments doivent respecter</span><div className="segmented-control"><button className={filterMode === "all" ? "active" : ""} onClick={() => setFilterMode("all")}>Toutes les conditions</button><button className={filterMode === "any" ? "active" : ""} onClick={() => setFilterMode("any")}>Au moins une</button></div></div><section className="partition-filter-section partition-rule-list"><div className="partition-rule-heading"><span>Conditions sur les informations</span><button onClick={addRule} disabled={!attributes.length}><Plus size={13} /> Ajouter</button></div>{filterRules.map((rule, index) => { const attribute = attributes.find((candidate) => candidate.id === rule.attributeId) ?? attributes[0]; const choices = attribute ? valuesForAttribute(attribute) : []; return <article className="partition-filter-rule" key={rule.id}><div className="partition-rule-number">{index + 1}</div><label><span>Information</span><div><select value={rule.attributeId} onChange={(event) => updateRule(rule.id, { attributeId: event.target.value, values: [] })}>{attributes.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select><ChevronDown size={14} /></div></label><label><span>Condition</span><div><select value={rule.operator} onChange={(event) => updateRule(rule.id, { operator: event.target.value as FilterRule["operator"] })}><option value="include">est parmi</option><option value="exclude">n’est pas parmi</option></select><ChevronDown size={14} /></div></label><div className="partition-rule-values"><span>Valeurs — plusieurs choix possibles</span>{choices.length ? <div>{choices.map((choice) => <label key={choice.value}><input type="checkbox" checked={rule.values.includes(choice.value)} onChange={() => updateRule(rule.id, { values: rule.values.includes(choice.value) ? rule.values.filter((value) => value !== choice.value) : [...rule.values, choice.value] })} /><span>{choice.label}</span></label>)}</div> : <small>Aucune valeur disponible dans les données.</small>}</div><button className="partition-remove-rule" onClick={() => setFilterRules((current) => current.filter((item) => item.id !== rule.id))} aria-label={`Supprimer la condition ${index + 1}`}><Trash2 size={14} /></button></article>; })}{!filterRules.length && <div className="partition-filter-empty">Ajoutez une condition pour croiser plusieurs informations.</div>}</section><button className="partition-reset-filters" disabled={!activeFilterCount && !filterRules.length} onClick={resetFilters}><RotateCcw size={14} /> Réinitialiser tous les filtres</button></aside>}
    ...
    {selected && <aside className="context-panel rf-context-panel partition-detail"><button className="panel-close" onClick={() => setSelectedId(null)} aria-label="Fermer le détail"><X size={16} /></button><p className="eyebrow">{selectedLevel?.label ?? selected.levelId}</p><h2>{selected.name}</h2>{selected.description && <p className="partition-detail-description">{selected.description}</p>}<dl className="detail-list">{selectedAttributes.map((attribute) => { const meta = valueMeta(selected.values[attribute.id], attribute, vocabularies); return <div key={attribute.id}><dt>{attribute.label}</dt><dd>{meta.label}{attribute.suffix}</dd></div>; })}</dl><div className="panel-section"><h3>Position dans le découpage</h3><div className="partition-breadcrumb">{ancestryOf(selected, allItems).map((item) => <span key={item.id}>{item.name}</span>)}</div></div>{relations.some((relation) => relation.sourceId === selected.id || relation.targetId === selected.id) && <div className="panel-section"><h3>Éléments liés</h3>{relations.filter((relation) => relation.sourceId === selected.id || relation.targetId === selected.id).map((relation) => { const otherId = relation.sourceId === selected.id ? relation.targetId : relation.sourceId; const other = allItems.find((item) => item.id === otherId); return other ? <button className="partition-related-row" key={relation.id} onClick={() => setSelectedId(other.id)}><span>{other.name}</span><small>{relation.type}</small></button> : null; })}</div>}</aside>}
  </section>;
}
