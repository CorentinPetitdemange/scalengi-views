...
export function PartitionView({ data, configuration }: ViewRendererProps) {
  ...
  return <section ref={frameRef} className="view-workspace rf-view-workspace partition-workspace" aria-label={t("Vue en découpage")}>
    <ViewToolbar className="partition-header" title={configuration?.label ?? t("Découpage personnalisé")} icon={<Layers3 size={16} />}><div className="partition-legend">{displayedLevels.map((level) => <span key={level.id}><i style={{ background: level.color }} />{level.label}</span>)}</div><div className="partition-filters"><label className="search-control"><Search size={15} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t("Rechercher un élément…")} /></label><button className={`partition-filter-button toolbar-icon-button ${filtersOpen || activeFilterCount ? "active" : ""}`} onClick={() => setFiltersOpen((current) => !current)} aria-label={t("Filtres")} title={t("Filtres")} aria-expanded={filtersOpen}><ListFilter size={15} />{activeFilterCount > 0 && <em>{activeFilterCount}</em>}</button></div><button className="rf-fullscreen-button toolbar-icon-button" aria-label={t(fullscreen ? "Quitter" : "Plein écran")} title={t(fullscreen ? "Quitter" : "Plein écran")} onClick={() => void toggleFullscreen()}>{fullscreen ? <Minimize2 size={16} /> : <Maximize2 size={16} />}</button></ViewToolbar>
