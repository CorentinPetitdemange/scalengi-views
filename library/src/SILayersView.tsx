...
export function SILayersView({ data, configuration }: { data: ViewDataset; configuration?: ViewConfiguration }) {
  ...
  return <section ref={frameRef} className="view-workspace si-layers-workspace impact-layers-workspace">
    <header className="si-layers-header"><div><p className="eyebrow">Analyse d’impact inter-couches</p><h1>{configuration?.label ?? "SI par couches"}</h1></div><div className="si-layers-actions">
      <label className="layers-search"><Search size={14} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Rechercher un point de départ" aria-label="Rechercher un élément" /></label>
    ...
    <div className="layers-summary impact-toolbar">{selected ? <><span className="impact-focus-label"><Target size={14} /><strong>{selected.name}</strong></span><div className="segmented-control" aria-label="Sens de l’analyse"><button className={direction === "incoming" ? "active" : ""} onClick={() => setDirection("incoming")}><ArrowUp size={12} /> Entrants</button><button className={direction === "both" ? "active" : ""} onClick={() => setDirection("both")}><ArrowDownUp size={12} /> Les deux</button><button className={direction === "outgoing" ? "active" : ""} onClick={() => setDirection("outgoing")}><ArrowDown size={12} /> Sortants</button></div><label className="impact-depth">Profondeur <select value={maxDepth} onChange={(event) => setMaxDepth(Number(event.target.value))}>{[1, 2, 3, 4].map((depth) => <option key={depth} value={depth}>{depth}</option>)}</select></label><button className="impact-reset" onClick={clearFocus}><RotateCcw size={13} /> Vue d’ensemble</button></> : <><span><Layers3 size={14} /><strong>{elements.length}</strong> objets</span><span><ArrowDownUp size={14} /><strong>{relations.length}</strong> relations</span><span className={orphanCount ? "warning" : ""}><Boxes size={14} /><strong>{orphanCount}</strong> objets isolés</span><em>Sélectionnez un objet pour isoler ses impacts entrants et sortants.</em></>}</div>
    {!elements.length ? <div className="layers-empty" data-view-export-content><Layers3 size={32} /><h2>Aucune architecture chargée</h2><p>Importez le modèle Excel pour analyser les dépendances entre les couches du SI.</p></div> : <div className="layers-body impact-layers-body" data-view-export-content>
      <main className="layer-stack impact-layer-stack" style={{ gridTemplateRows: `${selected ? "auto " : ""}repeat(${Math.max(1, layers.length)}, minmax(0, 1fr))` }}>{selected && <section className="impact-kpis"><article><ArrowUp size={15} /><span>Entrants</span><strong>{incomingCount}</strong><small>objets pouvant affecter le point focal</small></article><article><ArrowDown size={15} /><span>Sortants</span><strong>{outgoingCount}</strong><small>objets potentiellement affectés</small></article><article className={criticalCount ? "warning" : ""}><AlertTriangle size={15} /><span>Critiques</span><strong>{criticalCount}</strong><small>dépendances de criticité forte</small></article><article><CircleDot size={15} /><span>Relations visibles</span><strong>{visibleRelations.length}</strong><small>dans le périmètre analysé</small></article></section>}{layers.map((layer) => { const meta = layerMeta[layer]; const items = visible.filter((item) => item.layer === layer); return <section className="architecture-layer impact-layer" key={layer} style={{ "--layer-color": meta.color } as React.CSSProperties}><header><span>{meta.number}</span><div><strong>{meta.label}</strong><small>{meta.description}</small></div><em>{items.length}</em></header><div className="architecture-layer-items">{items.length ? items.map((item) => { const impact = scope.get(item.id); const directRelations = relations.filter((relation) => relation.sourceId === item.id || relation.targetId === item.id); return <button key={item.id} className={`${selectedId === item.id ? "selected " : ""}status-${statusClass(item.status)}`} onClick={() => setSelectedId(item.id)}><span className="layer-card-top"><i />{item.domain}</span><strong>{item.name}</strong><small>{item.owner || "Responsable non défini"}</small><span className="layer-card-foot"><em>{item.status}</em><b>{directRelations.length} lien{directRelations.length > 1 ? "s" : ""}</b></span>{impact && impact.direction !== "focus" && <span className={`impact-badge ${impact.direction}`}><b>{impact.direction === "incoming" ? "Entrant" : "Sortant"}</b> · niveau {impact.depth}</span>}{impact?.direction === "focus" && <span className="impact-badge focus"><Target size={10} /> Point focal</span>}</button>; }) : <div className="architecture-layer-empty">{selected ? "Aucun objet dans le périmètre d’impact" : "Aucun élément avec ces filtres"}</div>}</div></section>; })}</main>
      <aside className="layer-detail impact-detail">{selected ? <><div className="layer-detail-heading" style={{ "--layer-color": layerMeta[selected.layer]?.color ?? "#2563eb" } as React.CSSProperties}><span>Point focal · {layerMeta[selected.layer]?.label ?? selected.layer}</span><h2>{selected.name}</h2><p>{selected.description || "Aucune description renseignée."}</p></div><div className="layer-detail-facts"><div><span>Statut</span><strong className={`status-${statusClass(selected.status)}`}>{selected.status}</strong></div><div><span>Criticité</span><strong>{selected.criticality}</strong></div><div><span>Domaine</span><strong>{selected.domain}</strong></div><div><span>Responsable</span><strong>{selected.owner || "Non défini"}</strong></div></div><section><div className="layer-detail-section-title"><strong>Relations directes</strong><em>{selectedRelations.length}</em></div><div className="layer-relation-list">{selectedRelations.length ? selectedRelations.map((relation) => { const incoming = relation.targetId === selected.id; const otherId = incoming ? relation.sourceId : relation.targetId; const other = elements.find((item) => item.id === otherId); if (!other) return null; return <button key={relation.id} onClick={() => setSelectedId(other.id)}><span className={incoming ? "incoming" : "outgoing"}>{incoming ? "ENTRANT → POINT FOCAL" : "POINT FOCAL → SORTANT"}</span><strong>{other.name}</strong><small>{layerMeta[other.layer]?.label ?? other.layer} · {relation.relation}</small></button>; }) : <p>Aucune relation documentée pour cet élément.</p>}</div></section><section className="impact-reading-help"><strong>Comment lire cette analyse ?</strong><p>« Entrant » remonte vers les objets qui atteignent le point focal selon le sens des relations. « Sortant » suit les relations depuis le point focal vers les objets susceptibles d’être affectés.</p></section></> : <div className="layer-detail-placeholder"><Target size={24} /><strong>Choisissez le point de départ</strong><span>La vue isolera sa chaîne d’impact, couche par couche, avec le sens et la profondeur de chaque dépendance.</span></div>}</aside>
    </div>}
  </section>;
}
