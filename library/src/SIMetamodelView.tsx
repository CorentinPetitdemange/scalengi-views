...
export function SIMetamodelView({ configuration }: ViewRendererProps) {
  ...
  return <section ref={frameRef} className="view-workspace rf-view-workspace metamodel-workspace">
    <ViewToolbar className="metamodel-header" title={configuration?.label ?? t("Métamodèle du SI")} icon={<Layers3 size={16} />}><div className="metamodel-layer-filters">{layers.map((layer) => { const visible = visibleLayerIds.has(layer.id); return <button key={layer.id} className={visible ? "active" : ""} onClick={() => toggleLayer(layer.id)} style={{ "--metamodel-color": layer.color } as React.CSSProperties} aria-pressed={visible}><i />{layer.label}<strong>{allObjects.filter((item) => item.layerId === layer.id).length}</strong>{layer.layout === "transverse" && <em>{t(layer.side === "left" ? "Gauche" : "Droite")} · col. {layer.column}</em>}</button>; })}</div><label className="metamodel-density"><span>{t("Espacement")}</span><input type="range" min="1" max="10" step="1" value={density} onChange={(event) => setDensity(Number(event.target.value))} /></label><div className="metamodel-actions"><label className="layers-search"><Search size={14} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t("Rechercher un type…")} /></label><div className="segmented-control metamodel-relation-modes" aria-label={t("Affichage des relations")}><button className={relationMode === "hidden" ? "active" : ""} aria-label={t("Masquer les relations")} onClick={() => setRelationMode("hidden")} title={t("Masquer les relations")}><EyeOff size={14} /></button><button className={relationMode === "focus" ? "active" : ""} aria-label={t("Afficher uniquement les relations du type sélectionné")} onClick={() => setRelationMode("focus")} title={t("Afficher uniquement les relations du type sélectionné")}><Focus size={14} /></button><button className={relationMode === "all" ? "active" : ""} aria-label={t("Afficher toutes les relations")} onClick={() => setRelationMode("all")} title={t("Afficher toutes les relations")}><Eye size={14} /></button></div><button className="rf-fullscreen-button toolbar-icon-button" aria-label={t(fullscreen ? "Quitter" : "Plein écran")} title={t(fullscreen ? "Quitter" : "Plein écran")} onClick={() => void toggleFullscreen()}>{fullscreen ? <Minimize2 size={16} /> : <Maximize2 size={16} />}</button></div></ViewToolbar>
