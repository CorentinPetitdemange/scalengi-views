...
function RadarChart({ indicators, visibleSeries, selectedId, onSelect }: { indicators: UrbanisationIndicator[]; visibleSeries: Record<SeriesKey, boolean>; selectedId: string | null; onSelect: (indicator: UrbanisationIndicator) => void }) {
  ...
  if (indicators.length < 3) return <div className="radar-selection-empty"><Radar size={27} /><strong>Sélectionnez au moins trois dimensions</strong><span>Le radar a besoin de trois axes pour former une surface comparable.</span></div>;

  return <svg className="urbanisation-radar-svg" viewBox="0 0 780 610" role="img" aria-label={`Radar de maturité sur ${indicators.length} dimensions`}>
...
export function UrbanisationRadarView({ data, configuration }: { data: ViewDataset; configuration?: ViewConfiguration }) {
  ...
  return <section ref={frameRef} className="view-workspace urbanisation-workspace">
    <header className="urbanisation-header"><div><p className="eyebrow">Diagnostic d’urbanisation</p><h1>État et trajectoire du SI</h1></div><div className="urbanisation-header-actions">
      <label className="filter-control urbanisation-scope"><select value={scope} onChange={(event) => setScope(event.target.value as Scope)} aria-label="Dimensions affichées"><option value="priority">{priorityCount} écarts prioritaires</option><option value="all">Toutes les dimensions ({indicators.length})</option>{groups.map((group) => <option key={group} value={`group:${group}`}>{group}</option>)}<option value="custom">Sélection personnalisée ({customIds.size})</option></select><ChevronDown size={15} /></label>
      <button className={configurationOpen ? "secondary-action-button active" : "secondary-action-button"} onClick={() => setConfigurationOpen((value) => !value)}><Settings2 size={15} /> Paramétrer</button>
    ...
    <div className="urbanisation-legend">{(Object.keys(seriesMeta) as SeriesKey[]).map((key) => <button key={key} className={visibleSeries[key] ? "active" : ""} onClick={() => setVisibleSeries((current) => ({ ...current, [key]: !current[key] }))}><i style={{ background: seriesMeta[key].color }} />{seriesMeta[key].label}</button>)}<span>{visibleIndicators.length} dimension{visibleIndicators.length > 1 ? "s" : ""} affichée{visibleIndicators.length > 1 ? "s" : ""}</span></div>
    {!indicators.length ? <div className="urbanisation-empty" data-view-export-content><Radar size={32} /><h2>Aucune évaluation chargée</h2><p>Importez le modèle Excel de cette vue pour afficher votre diagnostic.</p></div> : <div className="urbanisation-body" data-view-export-content>
      <div className="radar-card"><div className="radar-card-heading"><div><span>Lecture synthétique</span><strong>{scope === "priority" ? "Priorités d’urbanisation" : scope === "all" ? "Ensemble du diagnostic" : scope === "custom" ? "Sélection personnalisée" : scope.slice(6)}</strong></div><small>Cliquez sur un axe pour ouvrir son détail</small></div><RadarChart indicators={visibleIndicators} visibleSeries={visibleSeries} selectedId={selectedId} onSelect={(item) => setSelectedId(item.id)} /></div>
      <aside className="urbanisation-insights"><div className="urbanisation-kpis"><article><Target size={15} /><span>Actuel</span><strong>{compact(average(indicators, "current"))}</strong></article><article><Focus size={15} /><span>Cible</span><strong>{compact(average(indicators, "target"))}</strong></article><article><Layers3 size={15} /><span>Cartographié</span><strong>{compact(average(indicators, "mapping"))}</strong></article><article className="gap-kpi"><Radar size={15} /><span>Écart pondéré</span><strong>{compact(weightedGap)}</strong></article></div>
        <div className="gap-list-heading"><div><strong>Écarts prioritaires</strong><span>Classés par écart × poids</span></div><em>{ranked.filter((item) => gapOf(item) > 0).length} à traiter</em></div>
        ...
        {selected && <div className="indicator-detail"><div><span>{selected.group}</span><strong>{selected.label}</strong></div><dl><div><dt>Actuel</dt><dd>{compact(selected.current)}</dd></div><div><dt>Cible</dt><dd>{compact(selected.target)}</dd></div><div><dt>Cartographie</dt><dd>{compact(selected.mapping)}</dd></div></dl>{selected.owner && <p><b>Responsable</b>{selected.owner}</p>}{selected.evidence && <p><b>Preuve</b>{selected.evidence}</p>}{selected.action && <p className="indicator-action"><b>Action recommandée</b>{selected.action}</p>}</div>}
      </aside>
    </div>}
    {configurationOpen && <aside className="urbanisation-config" aria-label="Paramètres de la vue"><button className="panel-close" onClick={() => setConfigurationOpen(false)} aria-label="Fermer les paramètres"><X /></button><p className="eyebrow">Paramètres de la vue</p><h2>Composer le diagnostic</h2><section><h3>Séries visibles</h3>{(Object.keys(seriesMeta) as SeriesKey[]).map((key) => <label key={key}><input type="checkbox" checked={visibleSeries[key]} onChange={() => setVisibleSeries((current) => ({ ...current, [key]: !current[key] }))} /><i style={{ background: seriesMeta[key].color }} /><span>{seriesMeta[key].label}</span></label>)}</section><section><div className="config-section-heading"><h3>Dimensions</h3><button onClick={() => { setCustomIds(new Set(indicators.map((item) => item.id))); setScope("custom"); }}>Tout sélectionner</button></div><div className="dimension-checklist">{indicators.map((indicator) => <label key={indicator.id}><input type="checkbox" checked={customIds.has(indicator.id)} onChange={() => toggleCustom(indicator.id)} /><span><strong>{indicator.label}</strong><small>{indicator.group}</small></span></label>)}</div></section></aside>}
  </section>;
}
