...
export function UrbanisationRadarView({ data, configuration }: { data: ViewDataset; configuration?: ViewConfiguration }) {
  ...
  return <section ref={frameRef} className="view-workspace urbanisation-workspace">
    <ViewToolbar className="urbanisation-header" title={t("État et trajectoire du SI")} icon={<Radar size={16} />}><div className="urbanisation-header-actions">
      ...
      <button className={configurationOpen ? "secondary-action-button toolbar-icon-button active" : "secondary-action-button toolbar-icon-button"} aria-label={t("Paramétrer")} title={t("Paramétrer")} onClick={() => setConfigurationOpen((value) => !value)}><Settings2 size={15} /></button>
      <button className="rf-fullscreen-button toolbar-icon-button" aria-label={t(fullscreen ? "Quitter" : "Plein écran")} title={t(fullscreen ? "Quitter" : "Plein écran")} onClick={() => void toggleFullscreen()}>{fullscreen ? <Minimize2 size={16} /> : <Maximize2 size={16} />}</button>
    </div></ViewToolbar>
