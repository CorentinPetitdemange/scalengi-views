...
export function AccountScreen({ user, onSessionChange }: { user: AuthUser; onSessionChange: (session: Session) => void }) {
  ...
  return <div className="account-page">
    ...
    <div className="account-grid">
      ...
      <form className="account-card" onSubmit={savePassword}><header><KeyRound size={19}/><div><h3>Mot de passe</h3><p>La modification ferme toutes vos autres sessions.</p></div></header><div className="account-card-body"><label><span>Mot de passe actuel</span><input type="password" autoComplete="current-password" required value={currentPassword} onChange={(event) => setCurrentPassword(event.target.value)}/></label><label><span>Nouveau mot de passe</span><input type="password" autoComplete="new-password" required minLength={12} maxLength={128} value={newPassword} onChange={(event) => setNewPassword(event.target.value)}/><small>12 caractères, majuscule, minuscule, chiffre et symbole.</small></label><label><span>Confirmer le mot de passe</span><input type="password" autoComplete="new-password" required value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)}/></label></div><footer><button className="primary-button" disabled={pending || !currentPassword || !newPassword || !confirmPassword} type="submit"><KeyRound size={15}/>Modifier</button></footer></form>
    </div>
  </div>;
}
