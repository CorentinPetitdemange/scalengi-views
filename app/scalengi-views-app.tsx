...
import { APP_LOCALES, configurationFromYaml, configurationToYaml, createConfigurationItem, downloadWorkbookTemplate, I18nProvider, isDatasetEmpty, MAX_YAML_BYTES, normalizeDataset, upgradeConfigurationSchema, useI18n, validateConfiguration, VIEW_CATALOG_GROUPS, viewRegistry, type AppLocale, type ViewConfiguration, type ViewDefinition } from "../library/src";
...
function ScalengiViewsShell() {
  ...
  const createInstance = async (definition: ViewDefinition, name: string, structure: "standard" | "blank", dataMode: "empty" | "demo", presetId?: string) => {
    ...
    const configuration = preset ? preset.createConfiguration() : structure === "standard" || dataMode === "demo" ? definition.createDefaultConfiguration() : definition.createBlankConfiguration();
  ...
  const title = screen === "settings" ? t("Paramètres") : screen === "instance" && activeInstance ? activeInstance.name : t("Mes vues");
  return (
    <div className={`app-shell theme-${theme} accent-${accent} ${collapsed ? "sidebar-collapsed" : ""}`}>
      <aside className="app-sidebar">
        ...
        <nav className="main-navigation" aria-label={t("Navigation principale")}>
          ...
          <button className="new-view-nav" onClick={() => setCreateOpen(true)} title={t("Créer une vue")}><Plus /><span>{t("Nouvelle vue")}</span></button>
...
function CreateViewModal({ onClose, onCreate }: { onClose: () => void; onCreate: (definition: ViewDefinition, name: string, structure: "standard" | "blank", dataMode: "empty" | "demo", presetId?: string) => Promise<void> }) {
  const { t } = useI18n();
  ...
  const [name, setName] = useState(() => definitions[0] ? `${definitions[0].shortTitle} — ${t("Nouvelle analyse")}` : "");
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) onClose(); }}><div className="modal create-view-modal" role="dialog" aria-modal="true" aria-label={t("Créer une vue")}><div className="modal-header"><div><p className="eyebrow">{t("Nouvelle analyse")}</p><h2>{t("Choisir un type de vue")}</h2></div><button className="icon-button" onClick={onClose} aria-label={t("Fermer")}><X size={18} /></button></div><div className="modal-body"><div className="create-definition-groups">{VIEW_CATALOG_GROUPS.map((group) => { const groupDefinitions = definitions.filter((definition) => definition.catalogGroup === group.id); if (!groupDefinitions.length) return null; return <section key={group.id}><header><div><strong>{group.label}</strong><p>{group.description}</p></div><span>{groupDefinitions.length}</span></header><div className="create-definition-grid">{groupDefinitions.map((definition) => { const Icon = iconFor(definition); return <button className={selectedId === definition.id ? "selected" : ""} key={definition.id} onClick={() => { setSelectedId(definition.id); setName(`${definition.shortTitle} — ${t("Nouvelle analyse")}`); setPresetId(definition.presets?.[0]?.id ?? ""); setStructure("standard"); setDataMode("demo"); }}><span className={`view-icon view-${definition.accent}`}><Icon size={20} /></span><div><strong>{definition.title}</strong><p>{definition.description}</p></div></button>; })}</div></section>; })}</div><label><span>{t("Nom de la vue")}</span><input value={name} onChange={(event) => setName(event.target.value)} autoFocus /></label>{selected?.presets?.length ? <div className="creation-preset-choice"><span>{t("Point de départ")}</span><div>{selected.presets.map((preset) => <button key={preset.id} className={presetId === preset.id ? "selected" : ""} onClick={() => { setPresetId(preset.id); setName(`${preset.title} — ${t("Nouvelle analyse")}`); const hasExample = Boolean(preset.createConfiguration().exampleData); if (!hasExample) setDataMode("empty"); }}><strong>{preset.title}</strong><small>{preset.description}</small></button>)}</div></div> : <div className="creation-structure-choice"><span>{t("Structure initiale")}</span><div><button className={structure === "standard" ? "selected" : ""} onClick={() => setStructure("standard")}><RotateCcw size={16} /><strong>{t("Modèle standard")}</strong><small>{t("Une base prête à adapter")}</small></button><button className={structure === "blank" ? "selected" : ""} onClick={() => { setStructure("blank"); setDataMode("empty"); }}><FileCode2 size={16} /><strong>{t("Page blanche")}</strong><small>{t("Tout définir vous-même")}</small></button></div></div>}<div className="creation-structure-choice"><span>{t("Données au démarrage")}</span><div><button className={dataMode === "demo" ? "selected" : ""} disabled={Boolean(selected?.presets?.length) && !presetHasExample} onClick={() => { setDataMode("demo"); setStructure("standard"); }}><Database size={16} /><strong>{t("Jeu d’exemple inclus")}</strong><small>{t(Boolean(selected?.presets?.length) && !presetHasExample ? "Non disponible pour une structure vierge" : "Activer l’exemple fourni par le YAML")}</small></button><button className={dataMode === "empty" ? "selected" : ""} onClick={() => setDataMode("empty")}><FileCode2 size={16} /><strong>{t("Aucune donnée")}</strong><small>{t("Importer votre fichier plus tard")}</small></button></div></div><div className="modal-note"><Database size={16} /><span>{t("Le modèle choisi contient son exemple dans le YAML. Il reste désactivable et ne cohabite jamais avec un fichier Excel actif.")}</span></div></div><div className="modal-footer"><button className="secondary-button" onClick={onClose}>{t("Annuler")}</button><button className="primary-button" disabled={!selected || !name.trim() || saving} onClick={() => { if (!selected) return; setSaving(true); void onCreate(selected, name.trim(), structure, dataMode, presetId || undefined).finally(() => setSaving(false)); }}><Plus size={16} /> {t(saving ? "Création…" : "Créer la vue")}</button></div></div></div>;
}
