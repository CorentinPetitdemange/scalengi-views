...
import { ArrowLeft, Boxes, Braces, CheckCircle2, CircleHelp, Cloud, Database, Download, FileCode2, FileSpreadsheet, GalleryVerticalEnd, Grid2X2, Layers3, LayoutDashboard, List, MapPinned, Moon, Network, PanelLeftClose, PanelLeftOpen, Plus, Radar as RadarIcon, RotateCcw, Route, Save, Settings, SlidersHorizontal, Star, Sun, Trash2, Upload, Users, X } from "lucide-react";
  ...
  return (
    <div className={`app-shell theme-${theme} accent-${accent} ${collapsed ? "sidebar-collapsed" : ""}`}>
      <aside className="app-sidebar">
        ...
        <div className="sidebar-footer"><div className="user-avatar">CM</div>{!collapsed && <div><strong>Espace de démonstration</strong><span>{APP_CHANNEL} · v{APP_VERSION}</span></div>}</div>
      </aside>
...
function InstanceStructureScreen({ instance, definition, onSave, onFlash }: { instance: ViewInstance; definition: ViewDefinition; onSave: (instance: ViewInstance) => Promise<void>; onFlash: (message: string) => void }) {
  ...
  const resetConfiguration = () => definition.presets?.find((preset) => preset.id === draft.options.preset)?.createConfiguration() ?? definition.createDefaultConfiguration();

  return <div className="instance-page structure-page"><div className="structure-hero"><div><p className="eyebrow">Configuration intrinsèque de la vue</p><h1>Structurer « {instance.name} »</h1><p>Cette structure décrit ce que la vue sait afficher. Les données seront liées ensuite dans un fichier Excel construit exactement à partir de cette configuration.</p></div><div className="structure-hero-actions"><button className="secondary-button" onClick={() => updateDraft(resetConfiguration())}><RotateCcw size={15} /> Réinitialiser le modèle</button><button className="secondary-button" onClick={() => updateDraft(definition.createBlankConfiguration())}><Trash2 size={15} /> Page blanche</button></div></div>
...
function CreateViewModal({ onClose, onCreate }: { onClose: () => void; onCreate: (definition: ViewDefinition, name: string, structure: "standard" | "blank", dataMode: "empty" | "demo", presetId?: string) => Promise<void> }) {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) onClose(); }}><div className="modal create-view-modal" role="dialog" aria-modal="true" aria-label="Créer une vue"><div className="modal-header"><div><p className="eyebrow">Nouvelle analyse</p><h2>Choisir un type de vue</h2></div><button className="icon-button" onClick={onClose}><X size={18} /></button></div><div className="modal-body"><div className="create-definition-groups">{VIEW_CATALOG_GROUPS.map((group) => { const groupDefinitions = definitions.filter((definition) => definition.catalogGroup === group.id); if (!groupDefinitions.length) return null; return <section key={group.id}><header><div><strong>{group.label}</strong><p>{group.description}</p></div><span>{groupDefinitions.length}</span></header><div className="create-definition-grid">{groupDefinitions.map((definition) => { const Icon = iconFor(definition); return <button className={selectedId === definition.id ? "selected" : ""} key={definition.id} onClick={() => { setSelectedId(definition.id); setName(`${definition.shortTitle} — Nouvelle analyse`); setPresetId(definition.presets?.[0]?.id ?? ""); setStructure("standard"); setDataMode("demo"); }}><span className={`view-icon view-${definition.accent}`}><Icon size={20} /></span><div><strong>{definition.title}</strong><p>{definition.description}</p></div></button>; })}</div></section>; })}</div><label><span>Nom de la vue</span><input value={name} onChange={(event) => setName(event.target.value)} autoFocus /></label>{selected?.presets?.length ? <div className="creation-preset-choice"><span>Point de départ</span><div>{selected.presets.map((preset) => <button key={preset.id} className={presetId === preset.id ? "selected" : ""} onClick={() => { setPresetId(preset.id); setName(`${preset.title} — Nouvelle analyse`); const hasExample = Boolean(preset.createConfiguration().exampleData); if (!hasExample) setDataMode("empty"); }}><strong>{preset.title}</strong><small>{preset.description}</small></button>)}</div></div> : <div className="creation-structure-choice"><span>Structure initiale</span><div><button className={structure === "standard" ? "selected" : ""} onClick={() => setStructure("standard")}><RotateCcw size={16} /><strong>Modèle standard</strong><small>Une base prête à adapter</small></button><button className={structure === "blank" ? "selected" : ""} onClick={() => { setStructure("blank"); setDataMode("empty"); }}><FileCode2 size={16} /><strong>Page blanche</strong><small>Tout définir vous-même</small></button></div></div>}<div className="creation-structure-choice"><span>Données au démarrage</span><div><button className={dataMode === "demo" ? "selected" : ""} disabled={Boolean(selected?.presets?.length) && !presetHasExample} onClick={() => { setDataMode("demo"); setStructure("standard"); }}><Database size={16} /><strong>Jeu d’exemple inclus</strong><small>{Boolean(selected?.presets?.length) && !presetHasExample ? "Non disponible pour une structure vierge" : "Activer l’exemple fourni par le YAML"}</small></button><button className={dataMode === "empty" ? "selected" : ""} onClick={() => setDataMode("empty")}><FileCode2 size={16} /><strong>Aucune donnée</strong><small>Importer votre fichier plus tard</small></button></div></div><div className="modal-note"><Database size={16} /><span>Le modèle choisi contient son exemple dans le YAML. Il reste désactivable et ne cohabite jamais avec un fichier Excel actif.</span></div></div><div className="modal-footer"><button className="secondary-button" onClick={onClose}>Annuler</button><button className="primary-button" disabled={!selected || !name.trim() || saving} onClick={() => { if (!selected) return; setSaving(true); void onCreate(selected, name.trim(), structure, dataMode, presetId || undefined).finally(() => setSaving(false)); }}><Plus size={16} /> {saving ? "Création…" : "Créer la vue"}</button></div></div></div>;
}
